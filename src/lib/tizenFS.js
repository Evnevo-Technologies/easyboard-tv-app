...
// Minimal Tizen FileSystem helper for offline caching of media (per uniqueId)
// Works only on Samsung Tizen TV runtime where `window.tizen` is available
...
export function tizenIsAvailable() {
  const available = !!isTizen;
  // log('tizenIsAvailable ->', available);
  return available;
}

export function getAppWritableDir(uniqueId = 'default') {
  if (!isTizen) return null;
  const dir = `wgt-private/user_${uniqueId}`;
  // log('getAppWritableDir ->', dir);
  return dir;
}

// Returns a stable filename for a URL (basic hash substitute)
function toSafeName(url, uniqueId = '') {
  try {
    const u = String(url + ':' + uniqueId); 
    let hash = 0;
    for (let i = 0; i < u.length; i++) {
      hash = (hash * 31 + u.charCodeAt(i)) >>> 0;
    }
    const ext = url.split('?')[0].split('#')[0].split('.').pop() || 'bin';
    const name = `${hash}.${ext}`;
    // log('toSafeName', { url, uniqueId, name });
    return name;
  } catch (_) {
    return `file_${Date.now()}.bin`;
  }
...
// Resolve offline path for a URL if file exists, otherwise null
export async function resolveOfflinePath(url, uniqueId = 'default') {
  if (!isTizen || !url) return null;
  return new Promise((resolve) => {
    try {
      const fileName = toSafeName(url, uniqueId);
      const fullPath = `${getAppWritableDir(uniqueId)}/${fileName}`;
      // log('resolveOfflinePath:start', { url, fullPath });
      window.tizen.filesystem.resolve(fullPath, (file) => {
        try {
          const uri = file && typeof file.toURI === 'function' ? file.toURI() : null;
          // log('resolveOfflinePath:success', { url, uri });
          resolve(uri);
        } catch (_) { resolve(null); }
      }, () => { 
        // log('resolveOfflinePath:not_found', { url }); 
        resolve(null); 
      }, 'r');
    } catch (_) { resolve(null); }
  });
...
// Download and persist a URL to Tizen FS via Download API. Returns local URI on success.
export async function downloadToOffline(url, uniqueId = 'default') {
  if (!isTizen || !url) return null;
  return new Promise((resolve) => {
    try {
      if (!window.tizen.download) { 
        // log('download:api_unavailable'); 
        return resolve(null); 
      }
      const fileName = toSafeName(url, uniqueId);
      const destination = getAppWritableDir(uniqueId);
      // log('download:start', { url, destination, fileName });
      const request = new window.tizen.DownloadRequest(url, destination + '/', fileName);
      const listener = {
        oncompleted: function (id, fullPath) {
          try {
            window.tizen.filesystem.resolve(fullPath, (file) => {
              try {
                const uri = file && file.toURI ? file.toURI() : null;
                // log('download:completed', { id, fullPath, uri });
                resolve(uri);
              } catch (_) { resolve(null); }
            }, () => resolve(null), 'r');
          } catch (_) { resolve(null); }
        ...
        oncanceled: function (id) { 
          // log('download:canceled', { id }); 
        },
        onprogress: function (id, receivedSize, totalSize) { 
          // log('download:progress', { id, receivedSize, totalSize }); 
        },
      };
      window.tizen.download.start(request, listener);
    } catch (e) {
...
// Get a local path for URL: prefer existing offline file, otherwise optionally download
export async function getOrCacheOffline(url, { downloadIfMissing = true, uniqueId = 'default' } = {}) {
  if (!isTizen || !url) return null;
  try {
    // log('getOrCacheOffline:start', { url, uniqueId, downloadIfMissing });
    const existing = await resolveOfflinePath(url, uniqueId);
    if (existing) { 
      // log('getOrCacheOffline:hit', { url, uri: existing }); 
      return existing; 
    }
    if (!downloadIfMissing) { 
      // log('getOrCacheOffline:miss_no_download', { url }); 
      return null; 
    }
    const downloaded = await downloadToOffline(url, uniqueId);
    // log('getOrCacheOffline:downloaded', { url, uri: downloaded });
    return downloaded;
  } catch (_) {
    // log('getOrCacheOffline:error', { url });
    return null;
  }
}

// Prime cache for a device JSON structure (images/videos and background music)
export async function primeOfflineCache(deviceJson, uniqueId = 'default') {
  if (!isTizen || !deviceJson) return;
  try {
    // log('prime:start');
    const channel = deviceJson.channel || {};
    ...
    if (bgm) urls.push(bgm);
    const unique = Array.from(new Set(urls)).slice(0, 20);
    // log('prime:queue', { total: urls.length, unique: unique.length });
    await Promise.all(unique.map((u) => getOrCacheOffline(u, { downloadIfMissing: true, uniqueId })));
    // log('prime:done');
  } catch (_) {}
}
